...
export default function Process() {
  ...
  const steps = [
    {
      ...
      icon: (
        <svg className="w-5 h-5 sm:w-6 sm:h-6" fill="none" stroke="currentColor" strokeWidth={1.6} viewBox="0 0 24 24">
  ...
  return (
    <section className="relative py-12 sm:py-20 lg:py-28 bg-gray-50/70 overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative grid grid-cols-2 lg:grid-cols-4 gap-x-3 gap-y-8 sm:gap-8 lg:gap-6">
          ...
          {steps.map((step, i) => (
            <Reveal key={step.title} delay={i * 120}>
              <div className="relative text-center group px-1 sm:px-2">
                <div className="relative inline-flex mb-3.5 sm:mb-5">
                ...
                <h3 className="font-bold text-gray-900 text-sm sm:text-base leading-snug mb-1.5 sm:mb-2">{step.title}</h3>
                <p className="text-xs sm:text-sm text-gray-500 leading-relaxed max-w-[260px] mx-auto">{step.text}</p>
              </div>
            </Reveal>
          ))}
        </div>
