      ...
      {outOfStockCount > 0 && (
        <div className="bg-red-50 border border-red-200 rounded-xl px-5 py-4 mb-6 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-red-800">{outOfStockCount} produit(s) en rupture de stock</p>
              <p className="text-xs text-red-600">Vérifiez votre stock et réapprovisionnez les produits concernés.</p>
            </div>
          </div>
          <Link href="/admin/stock" className="text-sm font-medium text-red-700 hover:text-red-800 whitespace-nowrap">
            Voir le stock
          </Link>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
        {stats.map((stat) => (
          <Link
            key={stat.label}
            href={stat.href}
            className="admin-card p-4 hover:border-gray-300 hover:bg-gray-50 transition-all group"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="admin-card p-6">
          ...
          <div className="space-y-2">
            ...
            >
              <div className="w-8 h-8 rounded-lg bg-gray-100 flex items-center justify-center text-gray-700 group-hover:bg-gray-200 transition-colors">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
                </svg>
              </div>
              <span className="text-sm font-medium text-gray-700 group-hover:text-gray-900 transition-colors">Ajouter un produit</span>
            </Link>
            <Link
              href="/admin/messages"
              className="flex items-center gap-3 px-3 py-3 rounded-lg hover:bg-gray-50 transition-colors group"
            >
              ...
              <span className="text-sm font-medium text-gray-700 group-hover:text-gray-900 transition-colors">Voir les messages</span>
            </Link>
            <Link
              href="/admin/users"
              className="flex items-center gap-3 px-3 py-3 rounded-lg hover:bg-gray-50 transition-colors group"
            >
              ...
              <span className="text-sm font-medium text-gray-700 group-hover:text-gray-900 transition-colors">Ajouter un commercial</span>
            </Link>
            <Link
              href="/admin/orders"
              className="flex items-center gap-3 px-3 py-3 rounded-lg hover:bg-gray-50 transition-colors group"
            >
              ...
              <span className="text-sm font-medium text-gray-700 group-hover:text-gray-900 transition-colors">Voir les commandes</span>
            </Link>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 admin-card p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/admin/orders" className="text-xs text-gray-900 hover:text-gray-600 font-medium">
              Tout voir
            </Link>
          </div>

          {recentOrders.length === 0 ? (
            <p className="text-sm text-gray-500 py-4 text-center">Aucune commande pour le moment.</p>
          ) : (
              ...
              {recentOrders.map((order) => (
                <div key={order.id} className="flex items-center justify-between py-2">
                  ...
                  <div className="text-right flex items-center gap-3">
                    ...
                    <span className="text-sm font-semibold text-gray-900 w-24 text-right tabular-nums">
