...
export default function ProductDetailPage() {
  ...
  async function addToCart(productId: string) {
    ...
    setAdding(false);
    if (res.ok) {
      setAddedMsg(language === "fr" ? "Ajouté au panier !" : "Added to cart!");
  ...
  if (notFound || !product) {
    return (
      <>
        <Header />
        <main className="pt-20 min-h-screen bg-gray-950 flex items-center justify-center">
          <div className="text-center">
            <h1 className="text-2xl font-bold text-white mb-3">
              {language === "fr" ? "Produit non trouvé" : "Product not found"}
            </h1>
            <Link href="/products" className="text-purple-300 hover:text-purple-200 text-sm font-medium transition-colors">
  ...
  const qtyStepper = (small = false) => (
    <div className="flex items-center border border-white/15 rounded-xl bg-white/5">
      <button
        ...
        className={`${small ? "w-8 h-8 text-sm" : "w-9 h-9"} flex items-center justify-center text-gray-300 hover:text-white hover:bg-white/5 rounded-l-xl transition-colors`}
      >
        -
      </button>
      <span className={`${small ? "w-8" : "w-10"} text-center text-sm font-semibold text-white tabular-nums`}>{qty}</span>
      <button
        onClick={() => setQty((q) => q + 1)}
        className={`${small ? "w-8 h-8 text-sm" : "w-9 h-9"} flex items-center justify-center text-gray-300 hover:text-white hover:bg-white/5 rounded-r-xl transition-colors`}
      >
        +
      </button>
    </div>
  ...
  return (
    <>
      <Header />
      <main className={`pt-20 min-h-screen bg-gray-950 ${showStickyBar && isAuthenticated && product.priceTTC !== undefined && product.stock !== undefined && product.stock > 0 ? "pb-20 md:pb-0" : ""}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="flex items-center gap-1.5 text-sm text-gray-500 mb-6 flex-wrap">
            ...
            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" /></svg>
            <Link href="/products" className="hover:text-purple-300 transition-colors">{getName(product.category)}</Link>
            {product.family && (
              <>
                <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" /></svg>
                <span className="text-white font-medium">{getName(product.family)}</span>
              </>
            )}
          </div>
          ...
          <div className="grid lg:grid-cols-2 gap-10">
            {/* Image gallery */}
            <div>
              <div className="relative aspect-square bg-white rounded-2xl border border-white/10 overflow-hidden mb-3 shadow-lg shadow-black/30">
                {images.length > 0 ? (
                  <Image
                  ...
                  {images.map((img, idx) => (
                    <button
                      ...
                      className={`relative w-16 h-16 shrink-0 rounded-lg border-2 overflow-hidden bg-white transition-colors ${
                        idx === activeImage ? "border-purple-500" : "border-white/15 hover:border-white/40"
                      }`}
                    >
            ...
            {/* Product info */}
            <div>
              <div className="flex items-center gap-2 text-[12px] font-semibold uppercase tracking-wider text-purple-300 mb-3 flex-wrap">
                <span>{getName(product.category)}</span>
                {product.family && (
                  <>
                    <svg className="w-3 h-3 text-gray-600" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" /></svg>
              ...
              {product.reference && (
                <p className="text-sm text-gray-500 font-mono mb-5">REF: {product.reference}</p>
              )}

              {/* Price section */}
              <div ref={priceSectionRef} className="bg-white/[0.04] rounded-2xl border border-white/10 p-6 mb-6">
                {isAuthenticated && product.priceTTC !== undefined ? (
                  <div>
                    <p className="text-3xl font-bold text-purple-300 mb-1 tabular-nums">
                    ...
                    {product.stock !== undefined && product.stock > 0 && (
                      <div className="mt-5 pt-5 border-t border-white/10">
                        ...
                        {addedMsg && (
                          <p className="text-sm text-emerald-400 font-medium mt-3 flex items-center gap-1.5">
                    ...
                    <div className="flex items-center gap-3 mb-4">
                      ...
                      <div>
                        <p className="text-sm font-semibold text-white">
                          {language === "fr" ? "Prix réservé aux professionnels" : "Prices for professionals only"}
                        </p>
                        <p className="text-xs text-gray-400">
                          {language === "fr"
                            ? "Contactez un commercial pour obtenir vos identifiants"
                            : "Contact a sales rep to get your credentials"}
                        </p>
                      </div>
                    </div>
                    <div className="flex gap-2">
                      <Link href="/signin" className="btn-primary flex-1 px-4 py-2.5 text-sm">
                        {language === "fr" ? "Se connecter" : "Sign in"}
                      </Link>
                      <Link href="/contact" className="btn-outline-light flex-1 px-4 py-2.5 text-sm">
              ...
              <Link
                href="/products"
                className="inline-flex items-center gap-2 text-sm text-purple-300 hover:text-purple-200 font-medium transition-colors"
              >
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                {related.map((r) => (
                  <Link
                    ...
                    className="group bg-white/[0.04] rounded-2xl border border-white/10 hover:border-purple-400/50 hover:bg-white/[0.07] overflow-hidden transition-all duration-300"
                  >
                    ...
                    <div className="p-3.5">
                      <p className="text-sm font-semibold text-white group-hover:text-purple-200 transition-colors line-clamp-2">{getName(r)}</p>
                      {isAuthenticated && r.priceTTC !== undefined && (
                        <p className="text-sm font-bold text-purple-300 mt-1 tabular-nums">{r.priceTTC.toFixed(2)} TND</p>
                      )}
                    </div>
                  </Link>
                ))}
      ...
      {isAuthenticated && product.priceTTC !== undefined && product.stock !== undefined && product.stock > 0 && showStickyBar && (
        <div className="fixed bottom-0 left-0 right-0 md:hidden z-40 bg-gray-950/95 backdrop-blur-md border-t border-white/10 shadow-[0_-4px_24px_rgba(0,0,0,0.5)] px-4 py-3">
          <div className="flex items-center gap-3">
            <p className="text-lg font-bold text-purple-300 shrink-0 tabular-nums">
